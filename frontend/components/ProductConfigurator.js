...
const ProductConfigurator = ({ productId }) => {
	...
	const updateAvailableOptions = useCallback(async () => {
		try {
			const response = await fetch(
				`${process.env.NEXT_PUBLIC_API_URL}/api/v1/products/${productId}/available_options`,
				{
					method: 'POST',
	...
	}, [selections, product, updateAvailableOptions]);

	const handleSelectionChange = (partId, choiceId) => {
		setSelections((prev) => ({ ...prev, [partId]: choiceId }));
	};
	...
	const handleAddToCart = async () => {
		...
		try {
			const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/cart_items`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ product_id: productId, selections })
			});
			const data = await response.json();
			if (data.success) {
	...
	const isConfigurationComplete = () => {
		...
		return product.parts.filter((part) => part.required).every((part) => selections[part.id] !== undefined);
	};
                      ...
                      `}
										>
											...
											<div className="flex justify-between items-center">
												...
												<div className="text-green-600 font-semibold">+€{choice.base_price}</div>
											</div>
											{!isAvailable && (
